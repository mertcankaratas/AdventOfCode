...
for (int i = 0; i <lstStr.Count; i++)
{
    ...
    lastDigit = -1;
    DigitString.Clear();
    foreach (var character in  lstStr[i])
    {
        if (firstDigit == -1)
        {
            if ((int)character > 47 && (int)character < 58)
            {
                firstDigit =  Convert.ToInt32(character.ToString());  
                DigitString.Clear();
            }
            else
            {
                DigitString.Append(character);
                if (ReturnDigit(DigitString.ToString()) != -1)
                {
                    firstDigit = ReturnDigit(DigitString.ToString());
                    DigitString.Clear();
                    DigitString.Append(character);
                }     
            }
        ...
        else if (firstDigit !=-1)
        {
            if ((int)character > 47 && (int)character < 58)
            {
                lastDigit =  Convert.ToInt32(character.ToString()); 
                DigitString.Clear();
            }
            else
            {
                ...
                if (ReturnDigit(DigitString.ToString()) != -1)
                {
                    lastDigit = ReturnDigit(DigitString.ToString());
                    DigitString.Clear();
                    DigitString.Append(character);
                }     
            }
...
int ReturnDigit(string characters)
{
    if (characters.Contains("one"))
    {
        return 1;
    }
    else if (characters.Contains("two"))
    {
        return 2;
    } else if (characters.Contains("three"))
    {
        return 3;
    }
    else if (characters.Contains("four"))
    {
        return 4;
    }
    else if (characters.Contains("five"))
    {
        return 5;
    } else if (characters.Contains("six"))
    {
        return 6;
    }
    else if (characters.Contains("seven"))
    {
        return 7;
    }
    
    else if (characters.Contains("eight"))
    {
        return 8;
    } else if (characters.Contains("nine"))
    {
        return 9;
    }
